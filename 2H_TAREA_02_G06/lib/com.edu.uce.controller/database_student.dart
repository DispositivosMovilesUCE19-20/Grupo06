...
class DBHelper2 {
  ...
  Future<io.File> writeJson(String data) async {
   ...
   void _createTableEst(Database db, int version) async {
    await db.execute(
      'CREATE TABLE Student(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT, lastname TEXT, email TEXT, celular TEXT, genero TEXT, fecha TEXT, foto TEXT)',
      
    );
    print('[DBHelper] _createTableEst: Success');
  }
  
  void saveEst(String name, String lastname, String email, String celular, String genero, String fecha, String foto) async {
    var dbClient = await db;
    await dbClient.transaction((trans) async {
      return await trans.rawInsert(
        'INSERT INTO Student(name, lastname, email, celular, genero, fecha, foto) VALUES(\'$name\', \'$lastname\', \'$email\', \'$celular\', \'$genero\', \'$fecha\', \'$foto\')',
      );
    });
    print(
